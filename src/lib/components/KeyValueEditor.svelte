...
<div class="flex flex-col gap-1">
  ...
  {#if items.length === 0}
    <p class="px-1 py-2 text-xs text-neutral-400">No items. Click + Add to create one.</p>
  {:else}
  ...
  <button
    type="button"
    class="mt-1 self-start rounded px-2 py-1 text-xs font-medium text-[#FF6C37] hover:bg-[#FF6C37]/10"
    onclick={addRow}
  >
    + Add
  </button>
