...
<aside
  class="flex h-full w-64 shrink-0 flex-col border-r border-slate-800/80 bg-slate-950/80"
>
  <div class="flex items-center gap-1 border-b border-slate-800/80 p-2">
    <button
      type="button"
  ...
  <div class="min-h-0 flex-1 overflow-y-auto p-2">
    {#if panel === "collections"}
      ...
      {#if collections.length === 0}
        <p class="px-2 py-6 text-center text-xs text-slate-500">
          No collections yet.
        </p>
      {/if}
      ...
      {#each collections as col (col.id)}
        <div class="mb-1">
          <div
            class="group flex items-center gap-1 rounded-md px-1.5 py-1 hover:bg-slate-800/60"
          >
            <button
              type="button"
          ...
          {#if isExpanded(col.id)}
            <ul class="ml-3 border-l border-slate-800 pl-1">
    ...
    {:else}
      ...
      {#each history as entry (entry.id)}
        <button
          type="button"
          class="mb-0.5 flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left hover:bg-slate-800/50"
          onclick={() => onselectHistory(entry)}
        >
          <span
            class="w-12 shrink-0 font-mono text-[10px] font-bold {METHOD_COLORS[
              entry.method
            ] ?? 'text-slate-400'}">{entry.method}</span
          >
          <div class="min-w-0 flex-1">
            <div class="truncate text-xs text-slate-300">{entry.url}</div>
  ...
  <div
    class="border-t border-slate-800/80 px-3 py-2 text-[10px] text-slate-500"
    title={workspacePath}
  >
...
{#snippet treeItem(
  ...
  >
    {#if item.type === "folder"}
      <div class="flex items-center gap-1 rounded-md px-2 py-1 hover:bg-slate-800/50">
        <button type="button" class="text-[10px] text-slate-500" onclick={() => toggle(item.id)}>
          {isExpanded(item.id) ? "▼" : "▶"}
        </button>
        {#if editingId === item.id}
          ...
          />
        {:else}
          <span class="min-w-0 flex-1 truncate text-xs text-slate-300">📁 {item.name}</span>
          <button
            type="button"
      ...
      <div class="flex items-center">
        ...
        >
          <span
            class="w-12 shrink-0 font-mono text-[10px] font-bold {METHOD_COLORS[
              item.request.method
            ] ?? 'text-slate-400'}">{item.request.method}</span
          >
          {#if editingId === item.id}
            <input
              class="input-field flex-1 py-0.5 text-xs"
